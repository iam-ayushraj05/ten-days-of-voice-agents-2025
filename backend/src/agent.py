...
async def player_action(
    ...
    # --- 💡 IMPROVEMENT: Context-Aware Action Handling ---
    chosen_key = None
    
    # Check for context-specific overrides before standard matching
    if current in ["tower", "tower_approach"] and "Found map fragment: 'Beneath the tower, the latch sings.'" in userdata.journal:
        # If the player tries to use the latch, map or open the hatch, AND they have the map, trigger the success scene.
        if any(keyword in action_text.lower() for keyword in ["latch", "hatch", "open", "map", "clue", "try"]):
            chosen_key = "open_hatch" # Overrides default 'try_latch_without_map'
            
    if current == "tower_combat" and "brass_key" in userdata.inventory:
        # If the player tries to use an item or fight, and has the key, give a bonus effect.
        if any(keyword in action_text.lower() for keyword in ["fight", "attack", "use key", "stand ground"]):
            # Set a temporary effect before proceeding with the intended action
            # Note: We don't advance the scene yet, we just enhance the description.
            userdata.journal.append("Used the brass key to create a brief, blinding flash during combat.")
            
    # --- End Contextual Override ---
    
    # Attempt 1: match exact action key
    if not chosen_key and action_text.lower() in (scene.get("choices") or {}):
        chosen_key = action_text.lower()

    # Attempt 2 & 3: fuzzy match (original logic, applied only if no contextual match)
    if not chosen_key:
    ...
    # --- 💡 IMPROVEMENT: Insert Combat Flavour Text ---
    flavour_text = ""
    temp_journal_entry = "Used the brass key to create a brief, blinding flash during combat."
    if temp_journal_entry in userdata.journal:
        flavour_text = "The brass key emits a blinding flash, momentarily stunning the creature! "
        # Remove the temporary journal entry to avoid repetition
        userdata.journal.remove(temp_journal_entry)
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🎲" * 8)
    logger.info("🚀 STARTING VOICE GAME MASTER (Brinmere Mini-Arc) - Developed by Ayush Raj")
