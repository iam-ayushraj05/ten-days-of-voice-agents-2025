...
# -------------------------
# Voice Profile Constants (for the Improv Host)
# -------------------------
ANNOUNCER_VOICE = {"voice": "en-US-marcus", "style": "Promo"} # High Energy, Announcer Style
CONVERSATIONAL_VOICE = {"voice": "en-US-matthew", "style": "Conversational"} # Calmer, Feedback Style
...
class Userdata:
    ...
    agent_session: Optional[AgentSession] = None # Added for dynamic voice switching
...
async def start_show(
    ...
    session = userdata.agent_session
    ...
    # --- IMPROVEMENT: Set Announcer voice for INTRO ---
    if session:
        session.tts.update_options(**ANNOUNCER_VOICE)
    ...
    # --- IMPROVEMENT: Set Conversational voice for the Improvisation Prompt ---
    if session:
        session.tts.update_options(**CONVERSATIONAL_VOICE)
...
async def next_scenario(ctx: RunContext[Userdata]) -> str:
    ...
    # --- IMPROVEMENT: Set Announcer voice for SCENE PROMPT ---
    if session:
        session.tts.update_options(**ANNOUNCER_VOICE)
...
async def record_performance(
    ...
    # --- IMPROVEMENT: Set Conversational voice for REACTION ---
    if session:
        session.tts.update_options(**CONVERSATIONAL_VOICE)
...
class GameMasterAgent(Agent):
    def __init__(self):
        instructions = """
        You are the host of a TV improv show called 'Improv Battle', presented by Ayush Raj.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🎭" * 6)
    logger.info("🚀 STARTING VOICE IMPROV HOST — Improv Battle (Presented by Ayush Raj)")
    ...
    session = AgentSession(
        ...
        tts=murf.TTS(
            voice="en-US-marcus",
            style="Promo", # Default Announcer Voice on startup
            text_pacing=True,
        ),
    ...
    # Inject session into userdata for tools to use for dynamic voice switching
